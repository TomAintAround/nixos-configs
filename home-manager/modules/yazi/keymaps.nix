{
	programs.yazi.keymap.mgr.prepend_keymap = [
		...
		{ on = [ "g" "v" ]; run = "cd ~/.var/app"; desc = "Go to ~/.var/app"; }
		...
		# Misc
		{ on = "<C-d>"; run = "shell -- ripdrag -x -i \"$1\""; desc = "Open drag and drop window"; }
		{ on = "<C-e>"; run = "shell -- aunpack \"$@\""; desc = "Extract files from selected file"; }
	];
}
