...
LSPCI="/var/lib/libvirt/bin/lspci"
...
function stop_display_manager_if_running {
	...
	if [[ -x /run/systemd/system ]] && echo "$DATE Distro is using Systemd"; then
		DISPMGR="$(grep 'ExecStart=' /etc/systemd/system/display-manager.service | $AWK -F'/' '{print $(NF-0)}' | sed 's/-start//' | sed 's/ //g')"
...
systemctl stop openrgb.service
...
if $LSPCI -nn | grep -e VGA | grep -s NVIDIA ; then
	...
	for i in $($LSOF 2>/dev/null | grep nvidia | $AWK '{print $1}' | $AWK '!seen[$0]++'); do
	  $PKILL -9 $i
	done
